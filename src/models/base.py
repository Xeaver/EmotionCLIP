...
class VisualTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, mask: torch.Tensor) -> torch.Tensor:
        ...
        x = torch.cat([x, x1], dim=1)  # shape = [*, grid ** 2 + 2, width]
...
def convert_weights_to_fp16(model: nn.Module):
    ...
    def _convert_weights_to_fp16(l: nn.Module):
...
def build_model_from_openai_state_dict(state_dict: dict) -> CLIP:
...
def trace_model(model: CLIP, batch_size: int = 256, device: torch.device = torch.device('cpu')):
    model.eval()
...
def resize_pos_embed(state_dict: OrderedDict, model: CLIP, interpolation: str = 'bicubic', seq_dim: int = 1):
...
class EmotionCLIP(nn.Module):
    def __init__(
        ...
        if backbone_checkpoint:
            ...
            logger.info(f'Backbone checkpoint loaded successfully.')
        else:
            logger.info(f'Backbone initialized with random weights.')
        ...
        # logit scale from backbone model
        self.logit_scale = self.backbone.logit_scale
        if reset_logit_scale:
            nn.init.constant_(self.logit_scale, np.log(1 / 0.07)) 


    def encode_video(self, video: torch.Tensor, video_mask: torch.Tensor) -> torch.Tensor:
        B, T, C, H, W = video.shape
        frames = video.reshape(-1, C, H, W)
        img_masks = video_mask.reshape(-1, H, W)
        x = self.backbone.encode_image(frames, img_masks)
        x = x.reshape(B, T, -1)
        x = self.visual_head(x)
        return x

    def encode_image(self, image: torch.Tensor, image_mask: torch.Tensor) -> torch.Tensor:
        return self.backbone.encode_image(image, image_mask)
    
    def encode_text(self, text: torch.Tensor) -> torch.Tensor:
        return self.backbone.encode_text(text)


    def forward(self, video: torch.Tensor, video_mask: torch.Tensor, text: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        """
        video: [B, T, 3, H, W]
        text: [B, 77]
        """
